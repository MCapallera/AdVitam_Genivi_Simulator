...
public class NetworkedCarConsole : MonoBehaviour
{
    ...
    public const float MPSTOMPH = 3.6f; //MPStoKPH
    ...
    void Update()
    {
        if (running)
        {
			...
			if (controller.MODE == true) {
				AutoPilotMode = "ON";
				AutoPilotStyle.normal.textColor = Color.green;
			} else {
				AutoPilotMode = "OFF";
				AutoPilotStyle.normal.textColor = Color.red;
			}
		}
    ...
    void OnGUI()
    {
        if (currentAlpha > 0.01f)
        {
			...
			//Marine
			GUI.Label(AutoPilotRect, AutoPilotMode, AutoPilotStyle); 
